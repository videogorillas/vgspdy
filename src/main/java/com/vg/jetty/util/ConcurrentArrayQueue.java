...
public class ConcurrentArrayQueue<T> extends AbstractQueue<T>
{
    ...
    private final AtomicReferenceArray<Block<T>> _blocks = new AtomicReferenceArray<Block<T>>(TAIL_OFFSET + 1);
    ...
    {
        return new Block<T>(getBlockSize());
    }
    ...
    protected static final class Block<E>
    {
        ...
        private final AtomicReference<Block<E>> next = new AtomicReference<Block<E>>();
        private final AtomicIntegerArray indexes = new AtomicIntegerArray(TAIL_OFFSET+1);

        protected Block(int blockSize)
        {
            elements = new AtomicReferenceArray<Object>(blockSize);
        }
