...
package com.vg.jetty.spdy.parser;

import java.nio.ByteBuffer;

import com.vg.jetty.spdy.CompressionFactory;
import com.vg.jetty.spdy.PushSynInfo;
...
{
    private static final byte _0b1100_0000 = (byte) (3 << 6);
    private static final byte _0b1110_0000 = (byte) (7 << 5);
        ...
        while (buffer.hasRemaining())
        {
            switch (state)
            {
                ...
                case ASSOCIATED_STREAM_ID:
                {
                    ...
                    if (buffer.remaining() >= 4)
                    {
                        associatedStreamId = buffer.getInt() & 0x7FFFFFFF;
    ...
    private byte readPriority(short version, byte currByte)
    {
        ...
        {
            case SPDY.V2:
                int p2 = currByte & (_0b1100_0000);
                p2 >>>= 6;
                return (byte)p2;
            case SPDY.V3:
                int p3 = currByte & (_0b1110_0000);
                p3 >>>= 5;
                return (byte)p3;
            default:
                throw new IllegalStateException();
