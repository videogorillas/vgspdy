...
public class SettingsBodyParser extends ControlFrameBodyParser
{
    ...
    private int convertIdAndFlags(short version, int idAndFlags)
    {
        switch (version)
        {
            case SPDY.V2:
            {
                ...
                int result = (idAndFlags & 0x000000FF) << 24;
                result += (idAndFlags & 0x0000FF00) << 8;
                result += (idAndFlags & 0x00FF0000) >>> 8;
                result += (idAndFlags & 0xFF000000) >>> 24;
                return result;
            }
            case SPDY.V3:
            {
    ...
    {
        int id = idAndFlags & 0x00FFFFFF;
        byte flags = (byte)((idAndFlags & 0xFF000000) >>> 24);
