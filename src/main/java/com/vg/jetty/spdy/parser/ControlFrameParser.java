...
{
    private final EnumMap<ControlFrameType, ControlFrameBodyParser> parsers = new EnumMap<ControlFrameType, ControlFrameBodyParser>(ControlFrameType.class);
        ...
        while (buffer.hasRemaining())
        {
            switch (state)
            {
                case VERSION:
                {
                    ...
                    {
                        version = (short)(buffer.getShort() & 0x7FFF);
                        state = State.TYPE;
                    }
                    else
                    {
